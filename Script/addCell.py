	...
	for index01, row01 in enumerate(list1):
		if row01[4].value == 'Y':
			...
			try:
				...
				sign = ref.get_start_sign(ref, u'添加单元')
				list2 = ref.cycle_index_list(ref, u'添加单元')
				for index02, row02 in enumerate(list2):

					if row02[5].value == 'Y':
						ref.writeValueToCell(dateTimeChineseFormat2(), index02 + sign + 2, 8)
						try:
							cellNum = row02[1].value
							buildArea = str(row02[2].value)
							area = str(row02[3].value)
							cellDescription = row02[4].value
							add_cell(driver, cellNum, buildArea, area, cellDescription)
							acp = AddCellPage(driver)
							if acp.information() == row02[6].value:
								ref.writeValueToCell('Pass', index02 + sign + 2, 9)
								add_cell_data(driver, cellNum)
								actual_file = listData + u'add_cell_data.log'
								expect_file = fileLibrary + u'add_cell_data.log'
