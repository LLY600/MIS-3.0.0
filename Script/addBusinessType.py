...
from Action.addBusinessType import *
	...
	for index01, row01 in enumerate(list1):
		if row01[4].value == 'Y':
			...
			try:
				...
				sign = ref.get_start_sign(ref, u'添加业态')
				list2 = ref.cycle_index_list(ref, u'添加业态')
				for index02, row02 in enumerate(list2):
					if row02[3].value == 'Y':
						ref.writeValueToCell(dateTimeChineseFormat2(), index02 + sign + 2, 6)
						try:
							businessTypeNum = row02[1].value
							businessTypeName = row02[2].value
							addBusinessType(driver, businessTypeNum, businessTypeName)
							abtp = AddBusinessTypePage(driver)
							if abtp.information() == row02[4].value:
