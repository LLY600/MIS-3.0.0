...
from Action.addBackPaymentType import *
...
def test_add_back_payment_type():
	...
	for index01, row01 in enumerate(list1):
		if row01[4].value == 'Y':
			...
			try:
				...
				sign = ref.get_start_sign(ref, u'后台收款')
				list2 = ref.cycle_index_list(ref, u'后台收款')
				for index02, row02 in enumerate(list2):
					if row02[3].value == 'Y':
						ref.writeValueToCell(dateTimeChineseFormat2(), index02 + sign + 2, 6)
						try:
							paymentTypeNum = row02[1].value
							paymentType = row02[2].value
							addBackPaymentType(driver, paymentTypeNum, paymentType)
							abptp = AddBackPaymentTypePage(driver)
							if abptp.information() == row02[4].value:
