...
from Action.addCommodityGroup import *
	...
	for index01, row01 in enumerate(list1):
		if row01[4].value == 'Y':
			...
			try:
				...
				sign = ref.get_start_sign(ref, u'货品组别')
				list2 = ref.cycle_index_list(ref, u'货品组别')
				for index02, row02 in enumerate(list2):
					if row02[3].value == 'Y':
						ref.writeValueToCell(dateTimeChineseFormat2(), index02 + sign + 2, 6)
						try:
							commodityGroupNum = row02[1].value
							commodityGroupName = row02[2].value
							addCommodityGroup(driver, commodityGroupNum, commodityGroupName)
							acgp = AddCommodityGroupPage(driver)
							if acgp.information() == row02[4].value:
