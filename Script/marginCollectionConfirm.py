...
from Action.confirmMarginCollection import *
...
def test_confirm_margin_collection():
	...
	for index01, row01 in enumerate(list1):
		if row01[4].value == 'Y':
			...
			try:
				...
				sign = ref.get_start_sign(ref, u'保证金收取确认')
				list2 = ref.cycle_index_list(ref, u'保证金收取确认')
				for index02, row02 in enumerate(list2):
					if row02[2].value == 'Y':
						...
						try:
							...
							file1 = listData + u'confirm_margin_collection_data.log'
							file2 = fileLibrary + u'confirm_margin_collection_data.log'
							if contain_file(file1, file2) == '一致':
								logger.info(u'对比文件一致，请确认列表数据！')
								confirm_margin_collection(driver)
								mccp = MarginCollectionConfirmPage(driver)
								if str(mccp.information()) == str(row02[3].value):
