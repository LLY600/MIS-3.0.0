...
from Action.confirmBrand import *
	...
	for index01, row01 in enumerate(list1):
		if row01[4].value == 'Y':
			...
			try:
				...
				sign = ref.get_start_sign(ref, u'确认品牌')
				list2 = ref.cycle_index_list(ref, u'确认品牌')
				for index02, row02 in enumerate(list2):
					if row02[2].value == 'Y':
						ref.writeValueToCell(dateTimeChineseFormat2(), index02 + sign + 2, 5)
						try:
							brand = row02[1].value
							get_brand_data(driver, brand)
							actual_file = listData + u'confirm_brand_data.log'
							expect_file = fileLibrary + u'confirm_brand_data.log'
							if contain_file(actual_file, expect_file) == '一致':
								logger.info(u'对比文件一致，请确认品牌数据！')
								confirm_brand(driver)
								cbp = ConfirmBrandPage(driver)
								if str(cbp.information()) == str(row02[3].value):
									ref.writeValueToCell('Pass', index02 + sign + 1, 6)
									logger.info(u'测试通过！')
								else:
									ref.writeValueToCell('Fail', index02 + sign + 1, 6)
									logger.info(u'测试失败！')
