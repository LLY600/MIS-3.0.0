...
from Action.addBuilding import *
...
def test_add_building():
	...
	# 如下注释部分为处理浏览器加载项，比如禁用flash、最大化窗口等，暂时未用。
	# chrome_options = Options()
	# profile = {'plugins.plugins_disabled': ['Adobe Flash Player']}  # chrome://components/
	...
	for index01, row01 in enumerate(list1):
		if row01[4].value == 'Y':
			...
			try:
				...
				sign = ref.get_start_sign(ref, u'添加楼宇')
				list2 = ref.cycle_index_list(ref, u'添加楼宇')
				for index02, row02 in enumerate(list2):
					if row02[5].value == 'Y':
						ref.writeValueToCell(dateTimeChineseFormat2(), index02 + sign + 2, 8)
						try:
							buildingNum = row02[1].value
							buildingName = row02[2].value
							area = str(row02[3].value)
							isBuilding = row02[4].value
							addBuilding(driver, buildingNum, buildingName, area, isBuilding)
							abp = AddBuildingPage(driver)
							if abp.information() == row02[6].value:
