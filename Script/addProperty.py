...
from Action.addProperty import *
	...
	for index01, row01 in enumerate(list1):
		if row01[4].value == 'Y':
			...
			try:
				...
				sign = ref.get_start_sign(ref, u'添加物业')
				list2 = ref.cycle_index_list(ref, u'添加物业')
				for index02, row02 in enumerate(list2):
					if row02[3].value == 'Y':
						ref.writeValueToCell(dateTimeChineseFormat2(), index02 + sign + 2, 6)
						try:
							propertyNum = row02[1].value
							propertyName = row02[2].value
							addProperty(driver, propertyNum, propertyName)
							app = AddPropertyPage(driver)
							if app.information() == row02[4].value:
