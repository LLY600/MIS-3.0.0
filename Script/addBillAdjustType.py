...
from Action.addBillAdjustType import *
	...
	for index01, row01 in enumerate(list1):
		if row01[4].value == 'Y':
			...
			try:
				...
				sign = ref.get_start_sign(ref, u'调整类型')
				list2 = ref.cycle_index_list(ref, u'调整类型')
				for index02, row02 in enumerate(list2):
					if row02[3].value == 'Y':
						ref.writeValueToCell(dateTimeChineseFormat2(), index02 + sign + 2, 6)
						try:
							billAdjustNum = row02[1].value
							billAdjustName = row02[2].value
							addBillAdjustType(driver, billAdjustNum, billAdjustName)
							abatp = AddBillAdjustTypePage(driver)
							if abatp.information() == row02[4].value:
