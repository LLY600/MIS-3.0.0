...
from Action.confirmCell import *
	...
	for index01, row01 in enumerate(list1):
		if row01[4].value == 'Y':
			...
			try:
				...
				sign = ref.get_start_sign(ref, u'确认单元')
				list2 = ref.cycle_index_list(ref, u'确认单元')
				for index02, row02 in enumerate(list2):
					if row02[2].value == 'Y':
						ref.writeValueToCell(dateTimeChineseFormat2(), index02 + sign + 2, 5)
						try:
							cell_name = row02[1].value
							get_cell_data(driver, cell_name)
							actual_file = listData + u'confirm_cell_data.log'
							expect_file = fileLibrary + u'confirm_cell_data.log'
							if contain_file(actual_file, expect_file) == '一致':
								logger.info(u'对比文件一致，请确认列表数据！')
								confirm_cell(driver)
								ccp = ConfirmCellPage(driver)
								if str(ccp.information()) == str(row02[3].value):
